# godotengine/godot PR Audit Report — Janitor v8.8.1
...
**946 pull requests** entered the `godotengine/godot` merge queue during this audit window.
...
- **3.5%** of PRs intercepted upstream of human review (34 of 946)
- **1.4 hours** of senior-engineer triage time redirected to productive work
- **$2630** Total Economic Impact across audit window
- **3.4 kWh** of CI datacenter energy conserved

## Threat Intelligence Summary

- **1** Swarm clone pairs detected (Jaccard ≥ 0.70 structural similarity)
- **38895** language antipatterns found across all PRs
- **9** zombie dependencies re-introduced (declared in manifest, never imported)
- **15** Critical Threats (security antipattern or Swarm collision) — $150/intercept
- **7** Necrotic GC intercepts (bot-closeable dead-code) — $20/intercept
- **12** Structural Slop intercepts (slop_score > 0, no threat signal) — $20/intercept

## Top 10 PRs by Slop Score

*See `gauntlet_intelligence_report.pdf` for the full ranked table with antipattern breakdowns.*

Highest slop score in this audit: **750** (100-point gate = fail threshold).
...
| Metric | Value |
|--------|-------|
| Source Analyzed | 3807.42 MB |
| Egress to Control Plane | 0 KB |
| Exfiltration Ratio | 0.0% |
| Bytes Received | 0 bytes |

> **House Gatekeeper Attestation**: The Gatekeeper has verified that 0 source bytes left the runner. The analysis surface was 3992378091 bytes of diff content. The control plane received 0 bytes of signed structural metadata. Your source code remained on your infrastructure for the entire audit.
...
7. **Data Sanitization** — The engine fetched the 1000 most recent PR references. 54 malformed or empty merge commits were filtered from the dataset prior to analysis, resulting in a cryptographically precise audit of 946 pull requests.
...
| `godot_cbom.json` | CycloneDX v1.5 Cryptography Bill of Materials |
| `godot_intel.json` | Per-repo JSON with clone pairs and top-50 PR detail |
| `godot_vex.json` | CycloneDX v1.5 VEX — exploitability status for all scanned threat classes |
